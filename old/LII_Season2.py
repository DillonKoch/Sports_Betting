...
def game_info(game_id, league='NFL'):
    ...
    if league == 'NFL':
        ...
        base_link = 'https://www.espn.com/mens-college-basketball/game?gameId='
    elif league == 'NHL':
...
class Bettor:
    ...
    def betSpread(self, bet_date, game_date, bet, to_win, pick, spread, espn_id, isParlay=False, parlayNo=None,
                  league='NFL', moneyLine=None, home_name=None, away_name=None, home_record=None, away_record=None):
        if not isParlay:
            self.numBets += 1
        ...
        newRow = [self.name, self.numBets, parlayNo, 'Spread', bet_date, game_date, bet, to_win, home_name,
                  home_record, away_name, away_record, pick, None, None, spread, moneyLine, league, None, espn_id]

        self.allBets.loc[len(self.allBets)] = newRow

    def betMoneyLine(self, bet_date, game_date, bet, to_win, pick, moneyLine, espn_id, isParlay=False,
                     parlayNo=None, league='NFL', home_name=None, away_name=None, home_record=None, away_record=None):
        if not isParlay:
            self.numBets += 1
        ...
        newRow = [self.name, self.numBets, parlayNo, 'Money Line', bet_date, game_date, bet, to_win, home_name,
                  home_record, away_name, away_record, pick, None, None, None, moneyLine, league, None, espn_id]

        self.allBets.loc[len(self.allBets)] = newRow

    def betProp(self, bet_date, game_date, bet, to_win, pick, moneyLine, espn_id=False, outcome=None, isParlay=False,
                parlayNo=None, league='NFL', home_name=None, away_name=None, home_record=None, away_record=None):
        ...
        # replacing "home" and "away" with home or away team's actual name
        try:
            pick = pick.replace('home', home_name).replace('away', away_name)
        except BaseException:
            print('nothing to replace here!')

        newRow = [self.name, self.numBets, parlayNo, 'Prop', bet_date, game_date, bet, to_win, home_name,
                  home_record, away_name, away_record, pick, None, None, None, moneyLine, league, outcome, espn_id]
    ...
    def update(self):
        ...
        for i in range(num_rows):
            if self.allBets.Outcome[i] is None or self.allBets['Home Score'][i] is None:
