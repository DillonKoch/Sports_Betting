...
# Author: Dillon Koch
# -----
# Last Modified: Friday, 1st January 2021 8:42:40 pm
...
class PB_Navigator(ESB_Navigator):
    ...
    def find_bet_sps(self, main_page_sp):  # Top Level
        ...
        """
        bet_area_sp = main_page_sp.find_all('div', attrs={'class': 'facf5sk'})[0]
    ...
    def _multiple_extra_bets(self, link_sp):  # Specific Helper find_extra_bet_link
        ...
        return True if re.match(multiple_bets_comp, link_sp_text) else False
    ...
    def find_extra_bet_link(self, bet_sp):  # Top Level
        ...
        if self._multiple_extra_bets(link_sp):
            ...
            full_link = self.start_link + link_extension
            return [full_link.replace('//', '/')]
        return []

    def scrape_extra_bets(self):  # Top Level
        pass
    ...
    def run(self):  # Run
        ...
        for sport in ['Football', 'Basketball'][:4]:
            ...
            for bet_sp in bet_sps:
                self.pb_parser.run(bet_sp)
                # extra_bet_links += self.find_extra_bet_link(bet_sp)

            # TODO change the process of finding extra bet links to be in the parser instead
            # TODO parser finds link, also sends back info about what bet type it is
...
if __name__ == '__main__':
    ...
    self = x
    bet_sps = x.run()
    with open('pb_bet_sps.pickle', 'wb') as f:
        pickle.dump(bet_sps, f)
